...
// Package filter provides a set of filters useful with the
// otelmux.WithFilter() option to control which inbound requests are traced.
package filter // import
...
func Any(fs ...otelmux.Filter) otelmux.Filter {
...
func All(fs ...otelmux.Filter) otelmux.Filter {
	return func(r *http.Request) bool {
		for _, f := range fs {
			if !f(r) {
				return false
...
func None(fs ...otelmux.Filter) otelmux.Filter {
	return func(r *http.Request) bool {
		for _, f := range fs {
			if f(r) {
				return false
...
func Not(f otelmux.Filter) otelmux.Filter {
	return func(r *http.Request) bool {
		return !f(r)
	}
}
...
func Hostname(h string) otelmux.Filter {
	return func(r *http.Request) bool {
...
func Path(p string) otelmux.Filter {
	return func(r *http.Request) bool {
		return r.URL.Path == p
	}
}
...
func PathPrefix(p string) otelmux.Filter {
	return func(r *http.Request) bool {
...
func Query(k, v string) otelmux.Filter {
	return func(r *http.Request) bool {
...
func QueryContains(k, v string) otelmux.Filter {
	return func(r *http.Request) bool {
		for _, qv := range r.URL.Query()[k] {
...
func Method(m string) otelmux.Filter {
	return func(r *http.Request) bool {
		return m == r.Method
	}
}
